#include <netkit/tcp/tcp_server.hpp>
#include <netkit/tcp/tcp_stream.hpp>

netkit::tcp::tcp_server::tcp_server(netkit::socket::addr addr)
	: addr_(std::move(addr)), listener_(std::make_unique<socket::native::native_sync_listener>(addr_, socket::type::tcp)) {}

netkit::tcp::tcp_server::~tcp_server() {
	this->close();
}

void netkit::tcp::tcp_server::bind() {
	listener_->bind();
}

void netkit::tcp::tcp_server::listen(int backlog) {
	listener_->listen(backlog);
}

void netkit::tcp::tcp_server::listen() {
	listener_->listen(-1);
}

std::unique_ptr<netkit::tcp::tcp_stream>
netkit::tcp::tcp_server::accept() {
	auto socket = listener_->accept();

	return std::make_unique<netkit::tcp::tcp_stream>(
		std::move(socket)
	);
}

void netkit::tcp::tcp_server::close() noexcept {
	if (listener_)
		listener_->close();
}

const netkit::socket::addr& netkit::tcp::tcp_server::get_local_endpoint() const noexcept {
	return listener_->get_local_endpoint();
}