...
netkit::stream::ReadUntilData
netkit::stream::read_until(stream::basic_stream& client_sock, const std::string& delimiter) {
    ReadUntilData ret;
    char buffer[4096];
    ...
    while (true) {
        ...
        if (status == stream::stream_status::eof ||
            status == stream::stream_status::closed) {
            break;
            }
    }
...
netkit::stream::read_until(stream::basic_async_stream& client_sock, const std::string& delimiter) {
    ...
    while (true) {
        ...
        const auto [bytes, status] =
            co_await client_sock.read(buffer, sizeof(buffer));
